...
```
And the data faceted to common types of helicopter:
...
The B06, used by both news crews (in Maryland and Virginia) and the Park Police (in D.C.):

```{r}
copterspotter_B06 <- copterspotter_clean |>
  filter(Type == "B06")

copterspotter_B06_vis <- copterspotter_B06 |>
  count(Date)

copterspotter_B06_vis |>
  ggplot(aes(x = Date, y = n)) +
  geom_point() +
  geom_smooth() +
  ylab("Number of Park Police/News Copter Spots Per Day") +
  geom_vline(aes(xintercept = as.numeric(as.Date("2022-03-23"))), color = "indianred") +
  geom_vline(aes(xintercept = as.numeric(as.Date("2022-05-13"))), color = "indianred") +
  geom_vline(aes(xintercept = as.numeric(as.Date("2022-07-27"))), color = "indianred") 
```
